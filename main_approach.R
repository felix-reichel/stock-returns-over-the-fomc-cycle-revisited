...
cac40_idx = read.csv('cac-40-index-france-historical-chart-data.csv', 
  skip = 13, 
  stringsAsFactors = TRUE) # "Each data point represents the closing value for that trading day."
...
cac40_x1 <- as.Date(cac40_idx$date[1]) # UTC
cac40_x2 <- as.Date(cac40_idx$date[length(cac40_idx$date)]) # UTC

eu_ssr_x1 <- as.Date(daily_ssr_estimates$...2[1]) # UTC
eu_ssr_x2 <- as.Date(daily_ssr_estimates$...2[length(daily_ssr_estimates$...2)]) # UTC

x1 <- max(cac40_x1, eu_ssr_x1) # = x1
x2 <- min(cac40_x2, eu_ssr_x2) # = x2

# Generate a sequence for matching interval X
...
# TODO: Compare differences/sign of delta's
# TODO: Time series econometrics, analysis, etc.
